...
def analyze_content(text):
    ...
    6. flashcards: Сделай ровно 6-8 карточек для запоминания. Вопросы должны проверять не очевидные факты, а ключевые термины или интересные концепции.
    7. quiz: Ровно 10 тестовых вопросов с 4 вариантами ответов каждый.
    8. mind_map: Сделай логичную структуру:
       - 1 корневой узел ("id": "root", "type": "root").
       - От 3 до 5 узлов тем ("type": "topic") — ветви от корня (связаны с root).
       - Для каждого узла темы создай от 2 до 3 дочерних узлов подтем ("type": "subtopic"), содержащих ключевые понятия (связаны со своим родительским topic).
"""
    elif text_len < 18000:
        length_directives = """
    ТРЕБОВАНИЯ К ДЕТАЛИЗАЦИИ РАЗДЕЛОВ JSON:
    ...
    6. flashcards: Сделай строго 8-10 карточек для запоминания. Вопросы должны проверять ключевые термины, цифры или интересные концепции.
    7. quiz: Ровно 10 тестовых вопросов с 4 вариантами ответов каждый. Вопросы должны охватывать все части текста.
    8. mind_map: Сделай логичную, детальную иерархическую структуру (карту знаний) для визуализации связей:
       - 1 корневой узел ("id": "root", "type": "root").
       - От 4 до 5 узлов тем ("type": "topic") — ветви от корня (связаны с root).
       - Для каждого узла темы создай от 3 до 4 дочерних узлов подтем ("type": "subtopic"), содержащих ключевые понятия (связаны со своим родительским topic).
"""
    else:
        length_directives = """
    ТРЕБОВАНИЯ К ДЕТАЛИЗАЦИИ РАЗДЕЛОВ JSON:
    ...
    6. flashcards: Сделай строго 10-12 карточек для запоминания, охватывающих все тонкости, терминологию и числовые данные текста.
    7. quiz: Ровно 10 тестовых вопросов с 4 вариантами ответов каждый. Вопросы должны проверять не только базовые, но и тонкие концепции, встречающиеся на протяжении всего текста.
    8. mind_map: Сделай развернутую, детальную иерархическую структуру (карту знаний) для визуализации связей:
       - 1 корневой узел ("id": "root", "type": "root").
       - От 5 до 7 узлов тем ("type": "topic") — ветви от корня (связаны с root).
       - Для каждого узла темы создай от 3 до 5 дочерних узлов подтем ("type": "subtopic"), детально раскрывающих каждый раздел (связаны со своим родительским topic).
"""
