...
def analyze_content(text):
    ...
    2. image_query: Одно-два самых точных, главных ключевых слова на английском языке (например, название конкретной игры, узкий научный термин, имя главного героя, бренд или технология, например: 'Deadlock', 'Minecraft', 'Artificial Intelligence', 'Geometry'), по которым можно найти идеальное изображение по теме в интернете. Пиши только чистый поисковый запрос на английском языке без кавычек и лишних слов.
    3. summary: Структурированный обзор (минимум 3 абзаца текста). Должен содержать:
    ...
    4. key_topics: Выдели от 3 до 5 ключевых тем/разделов. Для каждой темы напиши:
       - title: Название темы.
    ...
    5. detailed_analysis: Глубокий, всесторонний аналитический отчет (минимум 4 детальных абзаца). Должен содержать:
       ...
       - Одну релевантную иллюстрацию в конце.
    6. takeaways: От 5 до 8 ключевых выверенных уроков, инсайтов или выводов, сформулированных в виде законченных утверждений.
"""
        num_flashcards = 6
    ...
    1. title: Ёмкое, профессиональное и привлекательное название анализа (не более 15 слов).
    2. image_query: Одно-два самых точных, главных ключевых слова на английском языке (например, название конкретной игры, узкий научный термин, имя главного героя, бренд или технология, например: 'Deadlock', 'Minecraft', 'Artificial Intelligence', 'Geometry'), по которым можно найти идеальное изображение по теме в интернете. Пиши только чистый поисковый запрос на английском языке без кавычек и лишних слов.
    3. summary: Максимально детальный структурированный обзор (минимум 4 развернутых абзаца текста). Должен содержать:
       ...
       - Одну релевантную иллюстрацию в конце.
    4. key_topics: Выдели строго от 5 до 6 ключевых тем/разделов. Для каждой темы напиши:
       - title: Название темы.
    ...
    5. detailed_analysis: Глубокий, всесторонний аналитический отчет (минимум 6 детальных абзацев). Должен содержать:
       ...
       - Одну релевантную иллюстрацию в конце.
    6. takeaways: Строго от 8 до 10 ключевых выверенных уроков, инсайтов или выводов, сформулированных в виде развернутых законченных утверждений.
"""
        num_flashcards = 8
    ...
    1. title: Ёмкое, профессиональное и привлекательное название анализа (не более 15 слов).
    2. image_query: Одно-два самых точных, главных ключевых слова на английском языке (например, название конкретной игры, узкий научный термин, имя главного героя, бренд или технология, например: 'Deadlock', 'Minecraft', 'Artificial Intelligence', 'Geometry'), по которым можно найти идеальное изображение по теме в интернете. Пиши только чистый поисковый запрос на английском языке без кавычек и лишних слов.
    3. summary: Фундаментальный структурированный обзор (минимум 5-6 масштабных, детальных абзацев текста). Должен содержать:
       ...
       - Одну релевантную иллюстрацию в конце.
    4. key_topics: Выдели строго от 6 до 8 ключевых тем/разделов. Для каждой темы напиши:
       - title: Название темы.
    ...
    5. detailed_analysis: Всесторонний, детальный и академически глубокий аналитический отчет (минимум 8 масштабных абзацев). Должен содержать:
       ...
       - Одну релевантную иллюстрацию в конце.
    6. takeaways: Строго от 10 до 12 ключевых глубоких уроков, инсайтов или выводов, сформулированных в виде емких, развернутых и интеллектуально богатых утверждений.
"""
        num_flashcards = 10
    ...
    2. Для каждого текстового поля (кроме ключей JSON, image_query и списков) ты должен вернуть ОБЪЕКТ с ключами "ru", "en", "kk".
    ...
    5. В summary и detailed_analysis КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО вставлять ссылки на изображения или теги ![...](...)  — фронтенд автоматически подбирает релевантные фотографии из интернета по названию анализа.
    ...
    # ------------------ ШАГ 3: ХРОНОЛОГИЧЕСКОЕ ДРЕВО (MINDMAP) ------------------
    print("⏳ [Шаг 3/3] Запуск генерации хронологического древа...")
    prompt_mindmap = f"""
    ТВОЯ РОЛЬ: Ты — эксперт по хронологическому контент-анализу, визуализации лекционных материалов и последовательного хода повествования.
    
    ТВОЯ ЗАДАЧА: Сформировать иерархическую структуру ХРОНОЛОГИЧЕСКОГО ДРЕВА контента (Timeline Chronology Tree) по предоставленному анализу контента. Древо должно отражать последовательный ход лекции/видео с временными метками (таймкодами) начала каждого раздела. Результат должен быть ОДНОВРЕМЕННО на трех языках: РУССКОМ (ru), АНГЛИЙСКОМ (en) и КАЗАХСКОМ (kk).

    КРИТИЧЕСКИЕ ТРЕБОВАНИЯ К ХРОНОЛОГИЧЕСКОМУ ДРЕВУ:
    1. Для каждого текстового поля (текст узла, категория, лейбл связи) ты должен вернуть ОБЪЕКТ с ключами "ru", "en", "kk".
    2.nodes: Создай 1 корневой узел ("id": "root", "type": "root", "category": {{"ru": "Корень", "en": "Root", "kk": "Түбір"}}) с общей темой лекции.
    3.nodes: Выдели последовательные хронологические разделы ("type": "topic", например: "topic_1", "topic_2", "topic_3"). Их текст ОБЯЗАТЕЛЬНО должен начинаться с таймкода начала раздела из видео в формате "ММ:СС — Название раздела" (например: "00:00 — Введение", "04:15 — Основы Deadlock", "12:30 — Тактика на линии").
    4.nodes: Для каждого хронологического раздела создай от {min_subtopics} до {max_subtopics} дочерних узлов деталей/подтем ("type": "subtopic", например: "subtopic_1_1", "subtopic_1_2"), раскрывающих ключевые тезисы этого отрезка времени.
    5.links (Линейная Хронология времени): 
       - Свяжи корневой узел с ПЕРВЫМ хронологическим разделом ("source": "root", "target": "topic_1").
       - Свяжи хронологические разделы ПОСЛЕДОВАТЕЛЬНО друг с другом в линейную цепочку времени (например: "topic_1" -> "topic_2", "topic_2" -> "topic_3" и т.д.). Это критично для образования ствола времени лекции!
       - Свяжи каждый хронологический раздел со всеми его дочерними деталями/подтемами (например: "topic_1" -> "subtopic_1_1", "topic_1" -> "subtopic_1_2").
    6. Все связи в списке "links" должны ссылаться на реально существующие id узлов в списке "nodes".
